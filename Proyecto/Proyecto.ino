...
//Inicializacion de valores para la Red Galaxy A04e2887. wlqf9683.
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)payload[0] == 'E') {
    ...
    message_to_whatsapp(msgDistancia);
...
void loop() {
  ...
  msgDistancia = ("El objeto se encuentra a " + numDistancia + " cm de distancia");
  ...
  numDistancia.toCharArray(dChar, 50);
  ...
  if (now - lastMsg > 2000) {
    ...
    Serial.println("");
    snprintf(msg, MSG_BUFFER_SIZE, "%s,%s,%s", tempChar, humidChar, dChar);
